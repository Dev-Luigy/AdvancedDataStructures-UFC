...
def load_data():
    ...
    try:
        ...
        print(
            "Error: performance_results.csv not found. Please run the benchmark first."
        )
        return None
...
def analyze_tree_rotations(df):
    ...
    rotation_summary = (
        tree_data.groupby(["Structure", "Operation"])["Rotations"]
        .agg(["mean", "sum", "count"])
        .round(2)
    )
    ...
    rb_rotations = tree_data[tree_data["Structure"] == "RedBlack Tree"][
        "Rotations"
    ].sum()
    ...
    print(
        f"Difference: {abs(avl_rotations - rb_rotations)} ({'AVL' if avl_rotations > rb_rotations else 'RedBlack'} has more)"
    )
...
def analyze_fixup_operations(df):
    ...
    print("\n=== INSERTION FIXUPS ===")
    insertion_fixups = (
        tree_data.groupby(["Structure", "Operation"])["Insertion_Fixups"]
        .agg(["mean", "sum"])
        .round(2)
    )
    ...
    print("\n=== DELETION FIXUPS ===")
    deletion_fixups = (
        tree_data.groupby(["Structure", "Operation"])["Deletion_Fixups"]
        .agg(["mean", "sum"])
        .round(2)
    )
    ...
    if not rb_data.empty:
        color_changes = (
            rb_data.groupby("Operation")["Color_Changes"].agg(["mean", "sum"]).round(2)
        )
        print(color_changes)
    else:
...
def analyze_search_performance(df):
    ...
    print("\n=== KEY COMPARISONS ===")
    comparison_summary = (
        search_data.groupby("Structure")["Key_Comparisons"]
        .agg(["mean", "min", "max"])
        .round(2)
    )
    ...
    print("\n=== SEARCH DEPTH ===")
    depth_summary = (
        search_data.groupby("Structure")["Search_Depth"]
        .agg(["mean", "min", "max"])
        .round(2)
    )
    print(depth_summary)
    ...
    print("\n=== NODES VISITED ===")
    nodes_summary = (
        search_data.groupby("Structure")["Nodes_Visited"]
        .agg(["mean", "min", "max"])
        .round(2)
    )
    print(nodes_summary)
...
def analyze_key_comparisons(df):
    ...
    # Group by structure and operation
    comparison_summary = (
        df.groupby(["Structure", "Operation"])["Key_Comparisons"]
        .agg(["mean", "sum"])
        .round(2)
    )
...
def analyze_execution_time(df):
    ...
    # Group by structure and operation
    time_summary = (
        df.groupby(["Structure", "Operation"])["Execution_Time_ms"]
        .agg(["mean", "min", "max"])
        .round(3)
    )
...
def performance_recommendations(df):
    ...
    print("\nGeneral Insights:")
    print("• Hash tables (OpenHashMap, ExternHashMap) have fastest insertions")
    print("• ExternHashMap has fewest key comparisons for searches")
    print("• Tree structures provide balanced performance with logarithmic guarantees")
    print("• AVL trees typically require more rotations but maintain stricter balance")
