 ...
 * Por fim, o tipo Key deve também ter uma std::hash<Key> especificada no
 * namespace std, ou caso contrario você deve fornecer uma.
 *
 * @tparam Key key type
 * @tparam Value value type
 * @tparam Hash hash function type
 */
template <typename T, typename ColisionType, typename Hash = std::hash<T>>
...
public:
   ...
   */
  HashTable(size_t tableSize = 19, float load_factor = 1.0) {
   ...
   */
  size_t bucket(const T &k) const { return hash_code(k); }
   ...
   * Se m for menor que o tamanho atual da tabela, a funcao nao tem nenhum
   * efeito. Um rehashing eh uma operacao de reconstrucao da tabela: Todos os
   * elementos no container sao rearranjados de acordo com o seu valor de
   * hashing dentro na nova tabela. Isto pode alterar a ordem de iteracao dos
   * elementos dentro do container. Operacoes de rehashing sao realizadas
   * automaticamente pelo container sempre que load_factor() ultrapassa o
   * m_max_load_factor.
   *
  ...
  void rehash(size_t m) {
    ...
    if (new_table_size > m_table_size) {
      std::vector<std::list<T>> old_vec;
      old_vec = m_table; // copia as chaves para uma nova tabela
      m_table
          .clear(); // apaga todas as chaves da tabela atual e deixa ela vazia
   ...
   * Ao remover o elemento, o numero de elementos eh decrementado em 1
   * unidade. Retorna um booleano indicando se a remocao foi realizada.
   *
   ...
   * @return Value& := valor associado a chave
   */
  const T &operator[](const T &k) const { return at(k); }
...
private:
   ...
   */
  size_t hash_code(const T &k) const { return m_hashing(k) % m_table_size; }
};
